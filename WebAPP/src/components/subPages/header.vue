...
<template>
  <header v-if="!openBarIsOpen" class="headerapp">
    <Stack
      class="pl4 pr4 pt3 pb3 width-100p backgroundapp-color-1"
      direction="row"
    ...
    >
      ...
      >
        ...
        <Txt
          v-for="(item, index) in pages.getListPage()"
          class="mr14 ml5 word-no-break"
          :key="index"
          :href="item.url"
          :color="item.active ? 3 : 2"
          bold
          :hover="3"
          :font="1"
          :xs="18"
  ...
  <div v-else class="openBar backgroundapp-color-1">
    <Stack
      class="openbar-content"
      direction="column"
      justifyContent="flex-start"
      alignItems="center"
    >
      <Stack
        class="width-100p height-fit-content mt5 pr4"
        direction="row-reverse"
        justifyContent="space-between"
        alignItems="center"
      >
      ...
      <Stack
        class="width-100p mt10 hr1 pr22 pl22"
        direction="column"
        justifyContent="flex-start"
        alignItems="center"
      >
        <div
          class="width-100p text-align-center mb4"
          @click="openBarIsOpen = false"
          v-for="(item, index) in pages.getListPage()"
          :key="index"
        >
          <Txt
            text-align="center"
            class="mb8 word-no-break"
            :href="item.url"
            :color="item.active ? 3 : 2"
            :hover="3"
            :font="2"
        ...
        <div class="width-100p mt4">
          <Button1
            ...
            class="mt4 pt3 pb3"
          >
            <Txt text-align="center" :font="2" :color="-1" bold :xs="26">{{
              data.header.login
            }}</Txt>
          </Button1>
        </div>
