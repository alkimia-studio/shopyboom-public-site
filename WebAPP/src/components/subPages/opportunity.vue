...
const validation = (): boolean => {
  ...
  if (!isNotEmpty(formData.value.email.value)) {
    if (isValid) error(`${data.errorMessages.required}: Email`)
...
const onSubmit = () => {
  if (validation()) sendRequest()
}
</script>
<template>
  <Stack
    :class="size.biggerThan(sizeType.md) ? 'pr35 pl35 pt40 pb40' : 'pr5 pl5 pt20 pb20'"
    direction="column"
  ...
  >
    <Grid class="width-100p" container :lg="2" :md="1">
      ...
      <Grid :lg="1" :md="1">
        <Stack
          class="width-100p"
          :class="size.biggerThan(sizeType.md) ? 'pl30' : 'pb8'"
          direction="column"
        ...
        >
          ...
          <input
            ...
            class="input1 textapp-color-1 font-1 borderapp-color-5 mt6"
          ...
          <div>
            ...
            >
              <Txt bold :font="1" :color="-1" :xs="16">{{
                data.opportunity.right.buttonRound
              }}</Txt>
            </Button1>
          </div>
        </Stack>
